...
def extract_appid_and_name_from_parquet(file_path):
    """Extract appid and game name from a parquet file"""
    try:
        # Read just a few rows to extract the app ID and any potential name field
        ...
        # Try to extract game name - check if these fields exist
        game_name = None
        # Look for potential name fields in the DataFrame
        ...
        return int(app_id), game_name
    except Exception as e:
        st.error(f"Error extracting app ID and name from file: {str(e)}")
        return None, None
...
def process_uploaded_files(uploaded_files, themes_file="game_themes.json"):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for _, row in report_df.iterrows():
            ...
            dislikes = int(row['dislikes_sum'])
            
            # Calculate sentiment metrics
            like_ratio = f"{(likes / total * 100):.1f}%" if total > 0 else '0%'
            dislike_ratio = f"{(dislikes / total * 100):.1f}%" if total > 0 else '0%'
            
            # Get sentiment-specific reviews
            positive_reviews = row['Positive_Reviews'] if 'Positive_Reviews' in row and isinstance(row['Positive_Reviews'], list) else []
            negative_reviews = row['Negative_Reviews'] if 'Negative_Reviews' in row and isinstance(row['Negative_Reviews'], list) else []
