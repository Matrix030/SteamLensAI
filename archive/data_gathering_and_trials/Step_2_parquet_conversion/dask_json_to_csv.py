# DASK -  This program converts all the json file into schema based CSV files.
import json
import os
import time
import pandas as pd
...
def process_app(app_id):
    """Process a single app's JSON data into CSV format
    ...
    try:
        ...
        if rows:
            ...
            # Save to CSV
            output_dir = "../../csv_output_indie"
            os.makedirs(output_dir, exist_ok=True)
            output_file = os.path.join(output_dir, f"{app_id}.csv")
            
            # Add metadata as comment lines at the top of the CSV
            with open(output_file, 'w', encoding='utf-8') as f:
                f.write(f"# app_id: {app_id}\n")
                f.write(f"# total_reviews: {len(reviews)}\n")
                f.write(f"# processing_timestamp: {time.time()}\n")
            
            # Write the dataframe to CSV, append to the file with metadata comments
            df.to_csv(output_file, mode='a', index=False, encoding='utf-8', escapechar='\\', quoting=1)
...
if __name__ == "__main__":
    ...
    # Create summary file
    with open("CSV_length_dask.txt", 'w') as file:
    ...
    total_reviews = 0
    with open("CSV_length_dask.txt", 'a') as file:
